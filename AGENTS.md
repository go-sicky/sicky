...
- **Key Features**:
  ...
  - Observability: Prometheus metrics (RED + infra, see §3.6), OpenTelemetry tracing (OTLP/gRPC, OTLP/HTTP, Stdout, Uptrace + B3 propagator), built-in structured logger.
...
- **`tracer.Tracer`**: Manages OpenTelemetry spans (OTLP/gRPC, OTLP/HTTP, Stdout, Uptrace). Fiber propagation lives in `server/fiber/propagation.go` (W3C+B3 extract, request-id generation).
...
- Prometheus metrics are defined in `metrics/metrics.go` as `sicky_`-prefixed `CounterVec`/`HistogramVec`/`GaugeVec` (server RED, client RED, broker, job, runner, registry, infra, manager) plus 3 collectors (`build_info`, `go_collector`, `process_collector`), all exported as package-level variables. The old unlabelled `num_*` counters were removed (breaking change).
- Each collector is registered in the `init()` function via `metrics.Register()`; helpers `ObserveServerRequest`/`ObserveClientRequest`/`ObserveBrokerPublish`/`ObserveBrokerHandler`/`ObserveJobRun`/`ObserveRegistryOp`/`ObserveInfraOp` (+ `CountInfraInit`/`SetInfraUp`) keep call sites to one line.
...
- Server observations happen in the access-log interceptor/middleware after the handler returns (status/code known): gRPC `server/grpc/logger.go`, Fiber `server/fiber/logger.go`, HTTP `server/http/logger.go`; TCP/UDP/WebSocket observe around the `OnData` handler loop (`server/tcp/tcp.go`, `server/udp/udp.go`, `server/websocket/websocket.go`) with rejections counted separately in `sicky_server_rejected_total{server,reason}`.
  - **Client metrics**: real RPCs only — gRPC `Invoke` (unary, method+code+duration) and stream opens, HTTP `Do` (method+host+code+duration). The `Call()` placeholders only bump `{method="noop"}` so they never pollute SLIs. TCP/UDP/WebSocket clients have no real transport yet.
  - **Broker metrics**: `sicky_broker_publish/handler/subscribe_total` + durations, labelled `{broker,topic,result}` (`ok/error/panic/dup/acked/nacked/requeued`); `sicky_broker_connected{broker}` gauge on Connect/Disconnect.
  - **Job metrics**: `sicky_job_runs_total{job,task,result=ok/error/timeout/panic}` + duration, `ticks_total`, `registered_total`, `running` gauge. Non-timeout cron handlers are wrapped too (panic re-panics after recording).
  - **Runner metrics**: `submit/try_submit/task_runs_total{result}` (`enqueued/full/full_timeout/dropped_*`), `queue_depth`/`inflight`/`workers` gauges, task duration.
...
- **Cardinality rules**: `topic`/`route`/`method` labels are full-fidelity by decision — watch `/metrics` line count after adding high-cardinality topics; `uri/url/dsn` must NEVER enter labels (use `RedactDSN` for logs only).
...
- `/logger`: Built-in structured logger (`GeneralLogger` at `logger/general.go`), gRPC logger adapter (`logger/grpc.go`).
...
- **Main Dependencies**:
  - `github.com/gofiber/fiber/v3 v3.5.0`: Web framework (Fiber + WebSocket servers) + `gofiber/contrib/v3/websocket v1.2.4` + `gofiber/contrib/v3/swaggo v1.0.10`. Fiber v3 notes: `fiber.Ctx` is an interface (handlers take `c fiber.Ctx`, never `*fiber.Ctx`); request-scoped values use generic `fiber.Locals[T]`; listener knobs (`ListenerNetwork`, `DisableStartupMessage`) live in `ListenConfig` passed to `app.Listener`; `ETag` is an `etag` middleware (not an App flag); `TrustProxy` defaults on (loopback/link-local/private) via `trust_proxy *bool`; `UserContext/SetUserContext` are gone (`Context/SetContext`, Ctx itself is a `context.Context`).
...
5.  **Wire Metrics**: When adding new server or client protocols, add a Prometheus counter in `metrics/metrics.go` and wire its `.Inc()` call at the appropriate insertion point (server: interceptor/middleware before handler or before `OnData` loop; client: first statement of `Call()`/`Invoke`). When adding infra/broker/job/runner/registry operations, use the matching `Observe*` helper (`ObserveInfraOp`, `ObserveBrokerPublish/Handler`, `ObserveJobRun`, `ObserveRegistryOp`) so counter+histogram stay paired. `server/fiber`'s built-in chain owns the `server="fiber"` series — never add a second counting middleware on the same app.
...
- [x] **D — dead code deleted** (per decision): `runtime/` removed entirely (zero importers); `service.go` legacy `Run`/`Shutdown` comments removed; `metrics.StartMetrics` + `metrics/config.go` removed. `mdns`, `grpc metadata` placeholder, scaffold skeletons stay deliberately. 2026-09-06: legacy `logger/fiber.go` + `tracer/fiber.go` Fiber middlewares deleted in the v3 migration (zero importers).
...
- ### Fixed in 2026-09-06 audit-fix pass (P0/P1/P2, API-compatible)
- [x] **WebSocket hardening**: `Origins []string` (empty = same-origin: browser Origin must match Host, non-browser always allowed; explicit list exact-matches, `*` = all) enforced in a pre-upgrade middleware (403 on mismatch); `MaxMessageBytes` (`0` = unlimited) wired to `SetReadLimit`; custom `RecoverHandler` logs panic+stack server-side and drops the pool entry (upstream default leaked the panic value to the client); half-TLS fail-fast `ErrIncompleteTLSConfig`; handlers moved to `atomic.Pointer` CAS snapshot; idle reaper wired in Start/Stop (`PingDuration`/`MaxIdleDuration` were dead config — `Purge` was never called); `Session.Send` serialized via `sendMu` + write deadline + nil-conn guard; `LastActive` guarded via `mu` (touch/lastActive); `OnConnect` error now rejects the connection; `SessionPool` init under `poolMu`; `ShutdownTimeout` (default 10s) bounds Stop (fiber `ShutdownWithTimeout` + listener backstop + bounded `wg` wait).
- [x] **gRPC server**: recovery no longer sends `debug.Stack()`/panic value to clients (generic `codes.Internal`, full detail logged server-side via `LoggerConfig`); access logger reports the real `status.Code(err)` instead of hardcoded 200 (unary + stream).
- [x] **Manager**: `sanitizeValue` now redacts `tls_key_pem/tls_cert_pem/key_pem/cert_pem/private_key(/_pem)/endpoint/cloud_url` (private key no longer leaks via `/config`); health checks capture infra clients once (TOCTOU nil-deref fixed); `Start` binds synchronously (bind failure returns error) and publishes `srv`+`running` atomically (orphaned-listener race fixed); `Stop` force-closes after failed Shutdown and bounds `wg.Wait`; `ManagerConfig.Validate` dry-runs all 8 mux path registrations (invalid/conflicting paths are a Start error, not a process panic); `/services` marshal errors are logged (no silent empty body).
- [x] **Orchestrator**: `Consul.Watch` idempotent via `sync.Once` (was double-started: explicit + `registry.Watch()`); purge ticker + SIGHUP reload goroutines joined via WaitGroup before teardown; registry-register failure now stops+deregisters that service (no undiscoverable zombies); shutdown only Stops/Deregisters actually-started services; all 5 wrapper categories join their errors into `Run()`'s return; `Init` no longer marks initialized on the version-print path; `ConfigUnmarshal(nil)` returns `ErrConfigNilTarget`.
- [x] **Registry**: top-level helpers (`Register/Deregister/CheckInstance/Load/Watch/Stop`) now take `rgMu.RLock` (mirrors broker); `cloneInstance` drops the Topic→Instance back-reference (the source graph is cyclic by design; the JSON cycle broke `/services` + all backends); consul/redis/local registration marshal with `json.Marshal` + error propagation (no more empty-string/0-byte corruption from `utils.JSONAny*` — those stay exported but deprecated); redis registry ping bounded to 5s + `rdb.Close()` on failure; consul watcher `Run` error is logged instead of discarded.
- [x] **Broker**: `infra.RedactDSN` exported and applied to nats/jetstream/nsq URLs (credentials no longer leak in errors/logs; nsq keeps plain host:port visible); jetstream error paths `nc.Close()` the dialed conn; nsq ping failure `p.Stop()`s and connect failure `consumer.Stop()`s; nats/jetstream `Subscribe` check+insert under one lock (TOCTOU gone); nats/jetstream/nsq `conn`/`handlers` access locked; all `_ = recover()` swallow points now log (nats/jetstream/nsq/ticker/cron/runner); **nsq panicking handler NAKs instead of acking (no more permanent message loss)**; jetstream panicking handler NAKs; cron timeout-watchdog goroutine recovers and reports; `Message.Raw()` falls back to body bytes on marshal failure.
- [x] **Runner/service**: `runner/static` restartable (Start rebuilds task/done channels and resets stopped — previously a restarted runner silently dropped every Task); `service/mcp` handlers + initialized flag locked (`snapshotHandlers`); `service/interactive` stdin goroutine guarded by `sync.Once`.
- [x] **TCP/UDP**: sessions removed from the pool the moment a connection ends (no more ghost sessions polluting `MaxSessions`/`Length`); UDP `Stop` force-purges the pool (`PurgeForce`) so restart re-fires OnConnect; hooks (`RunBefore/AfterStart`, `RunBeforeStop`) now run unlocked in all 6 servers (hook→`Addr()` self-deadlock trap gone); `Metadata()` takes `RLock` before `Clone()` everywhere (websocket now clones instead of returning the live map); bounded Stop via new `ShutdownTimeout` config (default 10s, tcp/udp/websocket); `New()` warns when production DoS guards are left at 0=off (tcp `read_timeout`/`max_sessions`, udp `max_sessions`/`max_packets_per_second`) — defaults unchanged by decision.
- [x] **HTTP/Fiber**: http `Metadata` middleware no longer overwrites the Propagation-generated request-id with "" (empty request-ids in access logs gone, double sanitize avoided); `statusRecorder` passthroughs `Hijacker/Pusher/ReaderFrom` (upgrade-style handlers work again); fiber logger/tracer middlewares no longer call `ErrorHandler` themselves — the fiber core now runs it exactly once per failing request (was 2-3×).
- [x] **Client**: gRPC `Invoke` no longer logs `args`/`reply` payloads (PII/credential exposure); `tracingClientStream` ends the span on `RecvMsg` error/`CloseSend` with `sync.Once` (per-stream goroutine+span leak gone, ctx-done kept as backstop); tcp/udp client `Connect/Disconnect` state under a mutex; client lifecycle wrappers (`BeforeConnect` etc.) documented deprecated (never invoked — API kept).
