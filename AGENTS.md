...
5.  **Wire Metrics**: When adding new server or client protocols, add a Prometheus counter in `metrics/metrics.go` and wire its `.Inc()` call at the appropriate insertion point (server: interceptor/middleware before handler or before `OnData` loop; client: first statement of `Call()`/`Invoke`). When adding infra/broker/job/runner/registry operations, use the matching `Observe*` helper (`ObserveInfraOp`, `ObserveBrokerPublish/Handler`, `ObserveJobRun`, `ObserveRegistryOp`) so counter+histogram stay paired. Never mount `logger.NewFiberMiddleware` (deprecated) alongside `server/fiber`'s built-in chain.
...
- [ ] Fiber double-count: `logger/fiber.go` `NewFiberMiddleware` is deprecated legacy (kept, not maintained, now counter-free) — never mount it alongside `server/fiber`'s built-in chain.
